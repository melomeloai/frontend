...
export const CreditInfo: React.FC = () => {
  ...
  const renderCreditValue = (value: number | undefined, variant: "default" | "secondary" | "outline" = "default") => {
    if (isLoading) {
      return (
  ...
  const totalCredits = credits ? credits.permanentCredits + credits.renewableCredits : undefined;
  const nextResetTime = credits ? new Date(credits.nextResetTime) : null;

  return (
    <div className="space-y-3">
      <div className="flex items-center gap-2">
        <Coins className={`h-4 w-4 ${isLoading ? 'text-muted-foreground animate-pulse' : 'text-primary'}`} />
      ...
      <div className="space-y-2">
        ...
        <div className="flex items-center justify-between pt-2 border-t">
          ...
          {renderCreditValue(totalCredits, "default")}
        </div>
      </div>
